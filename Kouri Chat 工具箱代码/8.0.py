...
class APIConfig:
    ...
    def read_config():
        try:
            ...
            return {"real_server_base_url": "https://api.siliconflow.cn/", "api_key": "", "model": "deepseek-ai/DeepSeek-V3", "messages": [], "image_config": {"generate_size": "512x512"}}
...
class APITester:
    ...
    def recognize_image(self, image_path):
        url = f'{self.base_url}/v1/vision/recognize'
...
class KouriChatToolbox:
    ...
    def setup_ui(self):
        ...
        custom_api_menu.add_command(label="设置图片生成尺寸", command=self.set_image_size)
        ...
        ttk.Label(config_frame, text="URL地址:").grid(row=0, column=0, sticky="w")
    ...
    def generate_image(self):
        ...
        try:
            ...
            # 保存图片按钮
            def save_image():
                file_path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG Files", "*.png"), ("JPEG Files", "*.jpg")], title="保存图片")
                if file_path:
    ...
    def show_help(self):
        help_text = (
            ...
            "     答：点击导出按钮，选择保存路径即可。\n"
            "   - 如何设置图片生成尺寸？\n"
            "     答：在自定义 API 菜单中选择“设置图片生成尺寸”，输入尺寸格式如 512x512。\n"
        )
